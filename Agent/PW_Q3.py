...
* https://mk.gov.tr ve https://katalog.mk.gov.tr dışına çıkma.
...
* Sayfanın ana başlığını ve linkleri gözlemle.
* Arama kutusunu kullan ve ismi verilen kitabı ara.
* İsmi verilen kitabı bulursan, yazar bilgisini de ekle.
* Sonuçları gösteren sayfaya git ve kitabın yer numarasını tespit et.
* Her adımda ne yaptığını açıkla ve raporu sonunda oluştur.
* Sadece ana domain veya kataloğa ait subdomain’i (https://katalog.mk.gov.tr) kullan. Başka sitelere gitme.
* Görev sonunda 1-100 arasında bir başarı puanı ver.
* Boş veya tahmin içerikli cevap verme.
"""

ACTION_PROMPT = """
GÖREV: "Nutuk" isimli kitabı kütüphane kataloğunda arayınız ve yer numarasını (Call number) bulunuz.
* Cevap vereceğin formata örnek: [yazar adı] tarafından yazılan "Nutuk" kitabının yer numarası [yer numarası] / [sınıflama sistemi (LC veya DDC)]
...
def save_logs():
    ...
    with open(f"q3_session_{timestamp}.txt","w",encoding="utf-8") as f:
        f.write("\n".join(terminal_log))
    with open(f"q3_session_{timestamp}.json","w",encoding="utf-8") as f:
