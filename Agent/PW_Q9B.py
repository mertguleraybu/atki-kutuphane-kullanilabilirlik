...
ACTION_PROMPT = """
GÖREV: Araştırmacının kütüphane personelinden destek isteyebileceği sayfayı bulunuz.
* Yalnızca site içindeki bilgilere dayanarak sonuç verin ve işlem zincirini kısaca özetleyin. Tahmin yürütmeyin.
...
def save_logs():
    ...
    with open(f"q9B_session_{timestamp}.txt","w",encoding="utf-8") as f: f.write("\n".join(terminal_log))
    with open(f"q9B_session_{timestamp}.json","w",encoding="utf-8") as f: json.dump(json_log,f,indent=2,ensure_ascii=False)
