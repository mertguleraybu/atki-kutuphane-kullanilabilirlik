...
TARGET = "https://mk.gov.tr/icerik/detay/teknoloji-atolyesi"
...
ACTION_PROMPT = """
Başlangıç URL: https://mk.gov.tr/icerik/detay/teknoloji-atolyesi

Görev:
1. Hangi web sitesinde olduğunuzu belirleyin.
2. Ana sayfaya geri dönün.

Kurallar:
- Sayfayı gerçekten ziyaret etmeden cevap vermeyin.
- En az bir navigation (click veya goto) yapmalısınız.
- Tahmin yapmayın, yalnızca gördüğünüz bilgileri kullanın.
- Eğer ana sayfaya ulaşamazsanız, başarısız olarak işaretleyin.

Çıktı:
- Site adı:
- Ana sayfaya dönüş adımları:
- Kanıt URL:
- Başarı (1/0):
...
def save_logs():
    ...
    with open(f"q1r_session_{timestamp}.txt","w",encoding="utf-8") as f:
        f.write("\n".join(terminal_log))
    with open(f"q1r_session_{timestamp}.json","w",encoding="utf-8") as f:
