...
# DOM ayrıştırması yapabilmesi için doğrudan bir içerik sayfasına gönderiyoruz
TARGET = "https://mk.gov.tr/icerik/detay/hakkimizda" 
...
ACTION_PROMPT = """
GÖREV: Bulunduğunuz içerik sayfasını inceleyin. Aşağıdaki öğeleri ayrı alanlar halinde çıkarın: 
1) Ana başlık 
2) Breadcrumb yapısı 
3) İlgili menü öğesi 
4) İçerik içi bağlantılar 
5) Kullanıcıyı yönlendiren yapısal ipuçları. 
* Gözlemlenemeyen öğeler için 'tespit edilemedi' yazın. 
* Ardından sayfanın yapısal açıklık ve izlenebilirlik düzeyini en fazla iki cümle ile değerlendirin. Tahmin üretmeyin.

ÇIKTI FORMATI:
Sonuç (Ayrıştırılan Öğeler ve Değerlendirme):
Kanıt URL:
...
def save_logs():
    ...
    with open(f"q10_session_{timestamp}.txt","w",encoding="utf-8") as f: f.write("\n".join(terminal_log))
    with open(f"q10_session_{timestamp}.json","w",encoding="utf-8") as f: json.dump(json_log,f,indent=2,ensure_ascii=False)
