...
ACTION_PROMPT = """
GÖREV: Cumhurbaşkanlığı Millet Kütüphanesi web sitesinde bir veritabanına uzaktan erişim için kullanıcıyı yönlendiren yolu bulunuz. 
* Ardından bu erişimin hangi sayfa, bağlantı ya da hizmet üzerinden sağlandığını ve kullanıcı açısından hangi temel adımların gerektiğini yalnızca site içindeki bilgilere dayanarak özetleyiniz. 
* EN AZ İKİ (2) ilgili kanıt URL veriniz. Tahmin yürütmeyiniz.
...
def save_logs():
    ...
    with open(f"q7_session_{timestamp}.txt","w",encoding="utf-8") as f: f.write("\n".join(terminal_log))
    with open(f"q7_session_{timestamp}.json","w",encoding="utf-8") as f: json.dump(json_log,f,indent=2,ensure_ascii=False)
