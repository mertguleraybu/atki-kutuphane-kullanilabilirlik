...
ACTION_PROMPT = """
GÖREV: Cumhurbaşkanlığı Millet Kütüphanesi'nin çalışma saatlerini ve fiziki adresini site üzerinden bulun.
Görev esnasında:
* Bulduğun bilgiyi açıkça yaz: örn. "Kütüphane çalışma saatleri: [Açılış-Kapanış], [Pazartesi-Cuma], Adres: [Cadde/Sokak], [Mahalle], [Semt/İlçe], [Şehir], [Posta Kodu]"
* Sadece 'buldum, baktım' gibi genel ifadeler kullanma.
* Kanıt URL'i ve menü örneğini ekle.
...
def save_logs():
    ...
    with open(f"q2_session_{timestamp}.txt","w",encoding="utf-8") as f:
        f.write("\n".join(terminal_log))
    with open(f"q2_session_{timestamp}.json","w",encoding="utf-8") as f:
