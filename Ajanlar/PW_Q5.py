...
ACTION_PROMPT = """
GÖREV: Site içerisinde yer alan iletişim formunu bulun ve inceleyin. Zorunlu alanları tespit edin ve bu alanlarda istenen bilgilerin mantıklı ve makul olup olmadığını değerlendirin.
(Örneğin: 'E-posta alanı → Mantıklı, kullanıcıya geri dönüş için gerekli.')
...
def save_logs():
    ...
    with open(f"q5_session_{timestamp}.txt","w",encoding="utf-8") as f:
        f.write("\n".join(terminal_log))
    with open(f"q5_session_{timestamp}.json","w",encoding="utf-8") as f:
